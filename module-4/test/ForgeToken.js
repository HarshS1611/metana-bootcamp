...
describe("ForgeTokenContract", function () {
    ...
    describe("Trade Token", function () {
        ...
        it("should burn forged tokens 3", async function () {
            const { forgeToken, addr1 } = await loadFixture(deployForgeFixture);
            await forgeToken.connect(addr1).freeMint(0);
            await time.increase(61);
            await forgeToken.connect(addr1).freeMint(1);
        ...
        it("should burn forged tokens 4", async function () {
            const { forgeToken, addr1 } = await loadFixture(deployForgeFixture);
            await forgeToken.connect(addr1).freeMint(1);
            await time.increase(61);
            await forgeToken.connect(addr1).freeMint(2);
            ...
            await forgeToken.connect(addr1).tradeToken(4, 1);

            const finalBalance0 = await forgeToken.balanceOf(addr1.address, 0);
            const finalBalance1 = await forgeToken.balanceOf(addr1.address, 1);
            const finalBalance2 = await forgeToken.balanceOf(addr1.address, 4);
        ...
        it("should burn forged tokens 5", async function () {
            const { forgeToken, addr1 } = await loadFixture(deployForgeFixture);
            await forgeToken.connect(addr1).freeMint(0);
            await time.increase(61);
            await forgeToken.connect(addr1).freeMint(2);
            ...
            const initialBalance0 = await forgeToken.balanceOf(addr1.address, 0);
            const initialBalance1 = await forgeToken.balanceOf(addr1.address, 2);

            await forgeToken.connect(addr1).tradeToken(5, 0);

            const finalBalance0 = await forgeToken.balanceOf(addr1.address, 0);
            const finalBalance1 = await forgeToken.balanceOf(addr1.address, 2);
            const finalBalance2 = await forgeToken.balanceOf(addr1.address, 5);
        ...
        it("should burn forged tokens 6", async function () {
            const { forgeToken, addr1 } = await loadFixture(deployForgeFixture);
            await forgeToken.connect(addr1).freeMint(0);
            await time.increase(61);
            await forgeToken.connect(addr1).freeMint(1);
            ...
            await forgeToken.connect(addr1).tradeToken(6, 1);
            ...
            expect(finalBalance1).to.equal(initialBalance1);
            expect(finalBalance2).to.equal(initialBalance2);
            expect(finalBalance3).to.equal(BigInt(0));
        }
        );
